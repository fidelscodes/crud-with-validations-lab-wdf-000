class Song < ActiveRecord::Base
  ...
  validate  :invalid_without_released_year?
  ...
  def invalid_without_released_year?
    if self.released && self.release_year.blank?
      errors.add(:released_year, "cannot be blank if the song has been released")
    end
  end
  ...
  def same_song_twice_in_year?
    ...
    if previous_song && previous_song.release_year == self.release_year
      errors.add(:title, "cannot release the same song twice in a year")
    end
  end
end
